...
1. Find the Beamer source: `ls drafts/slides/$ARGUMENTS*.tex`
2. Extract all `\begin{tikzpicture}` blocks from Beamer
3. Compare with `output/figures/$ARGUMENTS/extract_tikz.tex`
...
```bash
cd output/figures/$ARGUMENTS
```

### Step 2: Compile the extract_tikz.tex file
```bash
TEXINPUTS=../../drafts/latex_files:$TEXINPUTS xelatex -interaction=nonstopmode extract_tikz.tex
```
